...
dealOneFile()
{
  ...
  if $(is_link_to_qing_space "$myfile"); then
    ...
    if [[ $? -eq 0  ]]; then
      ...
      chmod +w "${myfile##*/}"
      echo "'$1' withdrawn from the qing space"
    else
      echo "fail to withdraw '$myfile', broken link, run 'git qing repair'"
    fi
    popd 1>/dev/null
  fi
}
...
if [[ -L "$mypath"  ]]; then
  ...
  echo "git-qing:searching for links ..."
  ...
  echo "'$mypath' is not a directory nor a link to a qing file"
  exit
fi
